...
- `grid_track`: list of grid segments `{ start_t, end_t, bpm, grid_subdiv, downbeat_t }`. The app is the **tempo authority**, so each segment's grid is exact, not estimated. Each segment is its **own independently-anchored lattice** (`downbeat_t + k·tick`, unbounded in both directions *within that segment's arithmetic*): a session is a *chain* of per-segment lattices, never one session-wide grid, and beat phase does not carry across a param change — the producer's click restarts from a fresh anchor, so adjacent segments' lattice indices have no defined relationship (never pool `k` or fold beat-relative positions across segments as if they shared a phase origin). Spans not covered by any segment are grid-empty = free play; an empty list = a fully free session. This is a *first-class renderable track* (drawn as gridlines on the roll), which is also why it can't live as buried MIDI tempo meta (→ reinforces 3b).
...
- *A click-param change closes the segment — and re-declares an open enrollment span (implemented 2026-07-19).* Changing BPM or subdivision while a grade span is open **closes the current grid segment and opens a new one** at the change point (firmware: the controller re-posts `grid_start`; the brain's fold splits it). The **same rule applies to an open enrollment span** — its click snapshot goes stale identically — so the controller re-declares it with the ref carried forward; the FSM auto-closes the old span, so the log carries `enroll_end` + `enroll_start` with the new snapshot. The capture rule only preserves what actually happened; how Layer C later folds/merges/identifies the resulting split spans is deliberately left open until Layer C is designed. ("Silently keep the stale grid" remains the one ill-defined option; the old v1 alternative of disallowing mid-span changes is dead — re-declare is implemented on both span kinds.)
...
- **Decisions:** BPM grouping **resolved (2026-07-19)** — group by exact declared `(bpm, subdiv)` forever; never bucket, merge, or infer "nearby" tempos together. Sparse groupings stay honest via the min-n rule (`None`/refusal below threshold, `n` always surfaced); a tempo ramp fragmenting into small exact groups is *correct behavior*, not a problem to smooth — Layer D may add a reporting threshold, never a merge. Calibration = a one-time measured hardware constant set in device config (F2, lands with roadmap Experiment 7); the engine's honest-null handling covers the interim.
...
| 8′ | `downbeat_t` anchoring | **Resolved:** a beat edge as written by the producer (firmware: first edge at/after the declaration); lattice unbounded **per segment** (a session = a chain of independently-anchored lattices; beat phase undefined across segment boundaries), so direction is grading-invariant; beat-anchoring (not subdivision) is the load-bearing property |
...
| — | BPM grouping (ex-"bucketing") | **Resolved (2026-07-19):** group by exact declared `(bpm, subdiv)` forever — no bucketing, merging, or nearby-tempo inference; sparse groups stay honest via min-n (`None` below threshold; `n` always surfaced). Fragmentation under a tempo ramp is correct behavior; Layer D may add a reporting threshold, never a merge |
| — | IOI's architectural role | **Resolved (2026-07-19):** a descriptive Tier-1 primitive, never a core grading metric — the grading narrative ("how well did the drummer play against the metronome?") is answered by deviation/rush-drag/consistency. IOI stays implemented as substrate for future analyzers; before Layer B lands, `ioi.pairs` gains **additional** per-segment/`(bpm, subdiv)`-scoped rows (additive; analyzer version bump) so beat-relative clustering has tempo context — Layer B composes on Tier 1, never a private event-stream path |
| — | Enrollment click-param change | **Resolved (2026-07-19, implemented):** a BPM/subdiv change while an enrollment span is open re-declares it at the change point (ref carried forward; FSM writes `enroll_end` + `enroll_start` with the fresh snapshot) — the same §5 rule as grid segments. Capture preserves what happened; higher-level enrollment semantics (folding, merging, groove identity across splits) deliberately stay open until Layer C is designed |
