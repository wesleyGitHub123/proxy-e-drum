...
- **Data contract:** meta = `{schema_version, session_id, start_iso, kit_profile_id, user_id, calibration_offset_ms|null}`; event = `{type,t,note,velocity[,channel]}`; ctrl = `{type,t,msg}`; grid_start = `{type,t,bpm,subdiv,downbeat_t}`; enroll_start adds `profile_ref`. Declarations fully schematized now (spec §3: "the schema decision is the irreversible part").
  ...
  - **Amendment (2026-07-13, additive — no version bump):** declaration lines (`grid_start`/`grid_end`/`bookmark`/`enroll_start`/`enroll_end`) may carry an optional `trigger_span: [t0, t1]` as the **last** key — the gesture trigger-hit bracket (capture spec §5; phase1-stats-plan F1). Absent = not gesture-produced; additive under the ignore-unknown-fields policy, so pre-F1 files and all prior fixtures are byte-unchanged.
